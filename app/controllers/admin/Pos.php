...
class Pos extends MY_Controller
{
    ...
    public function getProductDataByCode($code = NULL, $warehouse_id = NULL)
    {
        ...
        if ($row) {
            ...
            $pr = array('id' => sha1(uniqid(mt_rand(), true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options);
    ...
    public function ajaxproducts($category_id = NULL, $brand_id = NULL)
    {
        ...
        if (!empty($products)) {
            foreach ($products as $product) {
                ...
                $prods .= "<button id=\"product-" . $category_id . $count . "\" type=\"button\" value='" . $product->code . "' title=\"" . $product->name . "\" class=\"btn-prni btn-" . $this->pos_settings->product_button_color . " product pos-tip\" data-container=\"body\">";
                if (!empty($product->image))
                    $prods .= "<img src=\"" . base_url() . "assets/uploads/thumbs/" . $product->image . "\" alt=\"" . $product->name . "\" class='img-rounded' />";
                else
                    $prods .= "<img src=\"" . base_url() . "assets/uploads/thumbs/no_image.png\" alt=\"" . $product->name . "\" class='img-rounded' />";
                $prods .= "<span>" . character_limiter($product->name, 40) . "</span>";
                $prods .= "</button>";
