...
class Products extends MY_Controller
{
    ...
    /**
     * Its a helper to update all stock on each warehouse
     * @param int $warehouse_id
     */
    function update_wh_stock($warehouse_id = 1)
    {
        $products = $this->products_model->getAllProducts();
        foreach ($products as $product) {
            var_dump($product->quantity); exit;
            $updateQuantity = $this->products_model->updateQuantity($product->id, $warehouse_id, $product->quantity);
        }
